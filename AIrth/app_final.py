...
def create_map():
    ...
    # Group data centers by emission level for legend
    green_dcs = []
    yellow_dcs = []
    red_dcs = []
...
# Create enhanced interactive emission heatmap with GIS features
def create_emission_heatmap():
    ...
    # Add enhanced heatmap layer with better radius
    fig.add_trace(go.Densitymapbox(
        ...
        colorscale=[
            [0, 'rgba(16, 185, 129, 0.6)'],      # Green for low
            [0.3, 'rgba(245, 158, 11, 0.7)'],    # Orange for medium
            [0.7, 'rgba(239, 68, 68, 0.8)'],     # Red for high
            [1, 'rgba(220, 38, 38, 0.9)']        # Dark red for very high
        ],
        showscale=True,
        colorbar=dict(
            title=dict(
                text='<b>CO₂ Emissions</b><br>(tons/year)',
        ...
        hovertemplate='<b>Emission Density</b><br>%{z:,.0f} tons CO₂/year<extra></extra>',
        name='Emission Density'
    ))
    
    # Group data centers by emission level for better legend
    low_dcs = []
    medium_dcs = []
    high_dcs = []
    ...
    # Add interactive markers by category
    categories = [
        (low_dcs, '#10B981', '🟢 Low Emissions (<50k tons)', 'circle'),
        (medium_dcs, '#F59E0B', '🟡 Medium Emissions (50-150k)', 'circle'),
        (high_dcs, '#EF4444', '🔴 High Emissions (>150k)', 'circle')
    ]
    ...
    for dcs, color, legend_name, symbol in categories:
        if dcs:
            ...
            dc_sizes = [max(12, dc.power_mw / 10) for dc in dcs]
            
            hover_texts = []
            for dc in dcs:
                metrics = dc.get_metrics()
                hover_text = (
                    f"<b style='font-size:14px'>{dc.name}</b><br><br>"
                    f"<b>📊 Emissions:</b> {metrics['effective_emissions']:,.0f} tons CO₂/year<br>"
...
def create_emissions_chart():
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=df['name'], x=df['emissions'], name='Total', orientation='h', marker=dict(color='#EF4444')))
    fig.add_trace(go.Bar(y=df['name'], x=df['effective'], name='Effective', orientation='h', marker=dict(color='#10B981')))
    fig.update_layout(xaxis_title='CO₂ Emissions (tons/year)', barmode='group', paper_bgcolor='rgba(0,0,0,0)',
                     plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#F8FAFC'), height=400, showlegend=True,
...
def create_renewable_chart():
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=df['name'], x=df['renewable'], name='Renewable', orientation='h', marker=dict(color='#10B981')))
    fig.add_trace(go.Bar(y=df['name'], x=df['non_renewable'], name='Non-Renewable', orientation='h', marker=dict(color='#64748B')))
    fig.update_layout(xaxis_title='Energy Mix (%)', barmode='stack', paper_bgcolor='rgba(0,0,0,0)',
                     plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#F8FAFC'), height=400,
                     xaxis=dict(gridcolor='rgba(255, 255, 255, 0.1)'), yaxis=dict(gridcolor='rgba(255, 255, 255, 0.1)'))
    return fig
...
def create_forecast_chart(dc_name):
    ...
    # Calculate scenarios
    # Business as usual (BAU) - current trajectory
    ...
    # Create comprehensive forecast section
    return html.Div([
        # Header
        html.Div([
...
# Create recommendations with enhanced details
def create_recommendations(dc_name):
    dc = next((d for d in platform.data_centers if d.name == dc_name), None)
    if not dc:
        return html.Div("No data center selected")
    
    # Import enhanced reporting for detailed recommendations
...
def create_scope_emissions_chart(dc_name):
    ...
    # Create clean, readable donut chart
    fig = go.Figure()
...
# INNOVATIVE: Create heat island effect map with radiant gradient
def create_heat_island_map(dc_name):
    ...
    try:
        ...
        # Create radiant heat zones (concentric circles with decreasing intensity)
        ...
        for i in range(num_zones, 0, -1):
            # Calculate radius for this zone
            zone_radius = max_radius_km * (i / num_zones)
            # Calculate temperature for this zone (decreases with distance)
            zone_temp = max_temp_increase * (i / num_zones)
            # Calculate opacity (higher at center)
            ...
            # Color intensity (darker red at center)
            if i == num_zones:
...
# INNOVATIVE: Climate & Environmental Impact Analysis
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            ...
            ], className="glass-card animate-in", style={'minHeight': '600px'})
        ], md=6, className="mb-4"),
        dbc.Col([
            html.Div([
                html.Div([html.I(className="fas fa-chart-pie"), "Scope Emissions Breakdown"], className="section-title"),
                dcc.Graph(id='scope-emissions-chart', config={'displayModeBar': False})
            ], className="glass-card animate-in", style={'minHeight': '600px'})
        ], md=6, className="mb-4"),
    ]),
    ]),
